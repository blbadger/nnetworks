...
minibatch_size = 100 # global variable
trainset = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=transforms)
dataloader = torch.utils.data.DataLoader(trainset, batch_size=minibatch_size, shuffle=False)
...
class StableDiscriminator(nn.Module):
	...
	def __init__(self):
		...
		self.conv6 = nn.Conv2d(512, 1, 4, stride=1, padding=0)
...
class StableGenerator(nn.Module):
	...
	def __init__(self, minibatch_size):
		...
		self.conv3 = nn.ConvTranspose2d(256, 64, 4, stride=2, padding=1)
...
def train_dcgan_adversaries(dataloader, discriminator, discriminator_optimizer, generator, generator_optimizer, loss_fn, epochs):
	...
	for e in range(epochs):
		...
		if float(generator_loss.item) > 0:
		...
		inputs = fixed_outputs.reshape(minibatch_size, 3, 64, 64).permute(0, 2, 3, 1).cpu().detach().numpy()
