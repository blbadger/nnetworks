...
tokenizer = torch.hub.load('huggingface/pytorch-transformers', 'tokenizer', 'gpt2')# import standard libraries
...
def check_abbreviated_equality(original_input, generated_input):
	encoded_input = tokenizer.encode(original_input, return_tensors='pt').to(device)
	input_embedding = model.transformer.wte(encoded_input) 
	position_ids = torch.tensor([i for i in range(len(tokens))]).to(device)
	positional_embedding = model.transformer.wpe(position_ids)
	input_embedding += positional_embedding

	encoded_gen = tokenizer.encode(generated_input, return_tensors='pt').to(device)
	gen_embedding= model.transformer.wte(encoded_gen) 
	gen_embedding += positional_embedding

	next_token = torch.argmax(a_model(input_embedding)[0][:, -1])
	next_word = tokenizer.decode(next_token)
	print (f'Next input word: {next_word}')

	next_gen_token = torch.argmax(a_model(gen_embedding)[0][:, -1])
	next_gen_word = tokenizer.decode(next_gen_token)
	print (f'Next generated input word: {next_gen_word}')

	return next_word, next_gen_word

check_abbreviated_equality(prompt, output)