...
class ImageDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		image = torchvision.io.read_image(img_path, torchvision.io.ImageReadMode.GRAY) # convert image to tensor of ints
...
class FCnet(nn.Module):
	...
	def __init__(self):
		...
		super().__init__()
		self.input_transform = nn.Linear(256*256, 512)
...
class InvertedFC(nn.Module):
	...
	def __init__(self):
		super().__init__()
		self.input_transform = nn.Linear(512, 256*256)
...
def train_generative_adversaries(dataloader, discriminator, discriminator_optimizer, generator, generator_optimizer, loss_fn, epochs):
	...
	total_loss = 0
	start = time.time()
	fixed_input = torch.randn(16, 100)
	...
	for e in range(epochs):
		...
		for batch, (x, y) in enumerate(dataloader):
			...
			if count % 5 == 0:
				...
				inputs = fixed_outputs.reshape(minibatch_size, 256, 256).detach().numpy()
				show_batch(inputs, count // 5,  grayscale=True)
...
def train_colorgan_adversaries(dataloader, discriminator, discriminator_optimizer, generator, generator_optimizer, loss_fn, epochs):
	...
	for e in range(epochs):
		...
		for batch, (x, y) in enumerate(dataloader):
			...
			if count % 5 == 0:
				fixed_outputs = generator(fixed_input)
				inputs = fixed_outputs.reshape(16, 3, 256, 256).permute(0, 2, 3, 1).detach().numpy()
				show_batch(inputs, count // 5)
				print (generator.d2.bias.norm())
				print (discriminator_prediction)
				print (generator_loss)
...
torch.save(discriminator.state_dict(), 'trained_models/monoflower_fcdiscriminator.pth')
torch.save(generator.state_dict(), 'trained_models/monoflower_fcgenerator.pth')
